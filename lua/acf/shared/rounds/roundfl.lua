...
ACF.AmmoBlacklist["FL"] = { "AC", "RAC", "MG", "HMG", "GL", "ACMG" }

local Round = {}

Round.type = "Ammo" --Tells the spawn menu what entity to spawn
Round.name = "Canister (CAN)" --Human readable name
Round.model = "models/munitions/dart_100mm.mdl" --Shell flight model
Round.desc = "Canister is legitimately giant shotgun shells for cannons. Turns any infantry infront of you into a fine, mushy paste. Can also be used as point defense against infantry."
...
function Round.create( Gun, BulletData )
	...
	if Gun:GetClass() == "acf_ammo" then --if ammo is cooking off, shoot in random direction
		...
		for I = 1, BulletData["Flechettes"] do
			...
			FlechetteData["Flight"] = (MuzzleVec+Inaccuracy):GetNormalized() * BulletData["MuzzleVel"] * 25 + Gun:GetVelocity()
		...
		for I = 1, BulletData["Flechettes"] do
			...
			FlechetteData["Flight"] = (MuzzleVec+BaseSpread+AddSpread):GetNormalized() * BulletData["MuzzleVel"] * 25 + Gun:GetVelocity()
			ACF_CreateBullet( FlechetteData )
		end
	end
...
function Round.convert( Crate, PlayerData )
	...
	local PenAdj = 1.4 --higher means lower pen, but more structure (hp) damage (old: 2.35, 2.85)
	local RadiusAdj = 1.0 -- lower means less structure (hp) damage, but higher pen (old: 1.0, 0.8)
	local PackRatio = 0.0025*Data["Flechettes"]+0.99 --how efficiently flechettes are packed into shell
	...
	Data["FlechetteArea"] = 3.1416 * Data["FlechetteRadius"]^1 -- area of a single flechette
	...
	Data["KETransfert"] = 0.8 								--Kinetic energy transfert to the target for movement purposes
	Data["Ricochet"] = 30								  		--Base ricochet angle
...
function Round.getDisplayData(Data, PlayerData)
	local GUIData = {}
	local Energy = ACF_Kinetic( Data["MuzzleVel"]*25 , Data["FlechetteMass"], Data["LimitVel"] )
...
function Round.propimpact( Index, Bullet, Target, HitNormal, HitPos, Bone )
	...
	if ACF_Check( Target ) then
		...
		if HitRes.Overkill > 0 then
			...
			Bullet["Flight"] = Bullet["Flight"]:GetNormalized() * (Energy.Kinetic*(1-HitRes.Loss)*2000/Bullet["ProjMass"])^0.5 * 25
			return "Penetrated"
